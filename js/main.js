...
function nodeActive(a) {
	...
	if (groupByDirection) {
		...
		f.push("<h2>Flux mutuels (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(mutual)) : f.push("Pas de flux mutuels<br>");
		size=Object.size(incoming);
		f.push("<h2>Flux entrants (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(incoming)) : f.push("Pas de flux entrants<br>");
		size=Object.size(outgoing);
		f.push("<h2>Flux sortants (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(outgoing)) : f.push("Pas de flux sortants<br>");
	} else {
